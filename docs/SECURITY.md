...
## Version 24.0 認証・利用者管理

- パスワードはハッシュ化して保存します。既存パスワードの復元・表示はできません。
- 管理者によるパスワード再設定では、仮パスワードを設定し、必要に応じて次回パスワード変更必須フラグを付与します。
- 利用者自身のパスワード変更では現在のパスワード確認、新パスワード、新パスワード確認を必須にします。
- パスワード変更、ロール変更、有効/無効変更、論理削除では `auth_version` を更新し、既存セッションを拒否します。
- 論理削除済みユーザー、無効ユーザー、Pilot対象外ユーザーはログインまたはAPI利用を拒否します。
- 監査ログには password、token、Authorization、API key、顧客本文全文、生成本文全文を保存しません。
- 監査ログには可能な範囲で `request_id`、`error_type`、`http_status`、Organization、Workspaceを保存します。
- メールによるパスワードリセットは未実装です。メール基盤導入後に本人確認フローを追加してください。
