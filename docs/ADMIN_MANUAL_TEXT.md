...
# Version 24.0 追加: ユーザー管理と監査

## ユーザーを作成する

1. 管理者ログインを行います。
2. 詳細モードまたは管理者メニューを開きます。
3. 「ユーザー管理」を開きます。
4. 氏名、メールアドレス、仮パスワード、Roleを入力します。
5. 「追加」を押します。

Roleは通常、一般利用者は `member`、閲覧のみは `viewer`、管理者は `admin` を選択します。`user` はAPI互換用で、内部的には `member` として保存されます。

## パスワードを再設定する

1. ユーザー一覧で対象ユーザーの新しい仮パスワードを入力します。
2. 「パスワード再設定」を押します。
3. 次回ログイン後にパスワード変更が必要な状態になります。
4. 仮パスワードは安全な社内手段で本人へ伝えてください。メール基盤による自動送信は未実装です。

## ユーザーを無効化・論理削除する

- 退職者や異動者はまず無効化します。
- 削除が必要な場合は「論理削除」を使います。
- 自分自身、および最後の有効adminは無効化・削除できません。

## 監査ログを見る

監査ログにはログイン、ログアウト、ユーザー作成、Role変更、有効/無効、Password再設定、AI生成、PPTX/PDF生成、Beautiful.ai生成、Quality Gate、エラーが記録されます。

Password、Token、APIキー、Authorization、顧客本文全文は保存しません。障害調査では `request_id`、`error_type`、`http_status` を確認してください。
