...
type AutoFlowStatus = "idle" | "typing" | "analyzing" | "question" | "generating" | "complete";
...
function buildAutoFlowMessage(status: AutoFlowStatus, result: AnalysisResponse | null, question?: ChatQuestion) {
  if (result) {
    return "完成しました。要約PPTをダウンロードできます。";
  }
  switch (status) {
    case "typing":
      return "入力が止まったら自動で整理します。";
    case "analyzing":
      return "AIが整理しています。";
    case "question":
    ...
    case "complete":
      return "完成しました。";
    default:
      return "案件メールを貼るだけで始まります。";
  }
}

const operationGuideSteps: Array<{ step: GuideStep; title: string }> = [
  { step: 1, title: "案件メール・議事録・URLを貼る" },
  { step: 2, title: "「AIに全部おまかせ」を押す" },
  { step: 3, title: "整理結果を確認する" },
  { step: 4, title: "「提案書を生成」を押す" },
...
const workModeGroups: Array<{ category: string; modes: WorkMode[] }> = [
  { category: "提案作成", modes: ["sales"] },
  { category: "商談支援", modes: ["coach", "reports"] },
  { category: "社内業務", modes: ["minutes", "mail", "tasks", "faq", "summary"] }
];

const workModeMap = new Map(workModeTabs.map((mode) => [mode.key, mode]));
...
function buildDashboardMetrics(history: HistoryEntry[], result: AnalysisResponse | null) {
  ...
  return [
    { label: "今日の生成数", value: `${todayCount}件`, note: "今日整理した案件" },
...
競合：${nextExtracted.competitor || "未抽出"}`
  ...
  async function generateProposal(sourceForm = form) {
    ...
    try {
      ...
      recordModeUsage("sales");
      recordUsage("提案書生成", allInputText(nextForm).length, "markdown", "success");
      void refreshAccountData();
    } catch (caught) {
      const friendly = toFriendlyError(caught);
      recordUsage("提案書生成", allInputText(nextForm).length, "markdown", "failure", friendly.category);
...
初回提案では、SEO課題、サイト構造、コンテンツ優先順位、KPI、概算費用を知りたい。`
  ...
  }, [isLoading, rawSourceText, result]);
  ...
  }, [assistantQuestionCount, autoFlowStatus, chatQuestionIndex, form, isLoading, rawSourceText, result]);
  ...
  return (
    ...
    <main className={`app-shell ${isDarkMode ? "dark-mode" : ""}`}>
      ...
      <section className="ai-wizard-shell" aria-label="AIウィザード">
        <div className="wizard-chat-card" aria-live="polite">
          ...
          <div className="wizard-chat-bubble">
            <span>Ready Crew Proposal AI</span>
            <p>{autoFlowMessage}</p>
          </div>
        </div>
            ...
            <div className="wizard-action-panel">
              <div className="wizard-primary-actions">
                ...
                >
                  ...
                  <span><strong>要約PPTをDL</strong><small>発表用</small></span>
                </button>
                <details className="wizard-other-format-menu">
                  ...
                  <div className="wizard-download-grid">
                    ...
                    <button className="ppt-download-button markdown" type="button" onClick={() => downloadMarkdown()}>
                      ...
                      <span><strong>Markdown</strong><small>原稿確認用</small></span>
                    </button>
                  </div>
                  <div className="wizard-detail-groups">
                    ...
                    <section>
                      <strong>連携予定</strong>
                      <button type="button" onClick={() => openDetailsPanel("future-integration-panel")}>MCP・Automations</button>
                    </section>
                    <section>
                      <strong>履歴</strong>
                      <button type="button" onClick={() => openDetailsPanel("dashboard-panel")}>生成履歴・CRM</button>
                    </section>
                  </div>
                </details>
              </div>
      ...
      <section className="one-screen-start" aria-label="初期入力">
        ...
        <div className="one-screen-links">
          ...
          >
            ...
            {isGuideEnabled && currentGuideStep === 5 && <span className="guide-label link-guide-label">DLはこちら</span>}
            整理結果を見る
          </button>
        </div>
      </section>
      ...
      <details className="advanced-foldout">
        <summary>CRMを開く</summary>
        <CrmPanel customers={crmCustomers} projects={crmProjects} />
      </details>
      </details>

      <details className="digital-coworker-panel advanced-foldout" id="company-research-panel" aria-label="AI Digital Coworker">
              ...
              <aside className="ppt-panel">
                ...
                <div className="ppt-button-stack">
                  ...
                  >
                    ...
                    <span>
                      <strong>{isDownloadingSummaryPowerPoint ? "要約版生成中" : "要約PPTをDL"}</strong>
                      <small>発表用</small>
                    </span>
                  </button>
                  <button
                  ...
                  >
                    ...
                    <span>
                      <strong>{isDownloadingPowerPoint ? "生成中" : "詳細PPTをDL"}</strong>
                      <small>詳細提案用</small>
                    </span>
                  </button>
                  <button
                  ...
                  >
                    ...
                    <span>
                      <strong>{isDownloadingEstimatePdf ? "PDF生成中" : "見積PDFをDL"}</strong>
                      <small>見積確認用</small>
                    </span>
                  </button>
                  <button className="ppt-download-button markdown" type="button" onClick={() => downloadMarkdown()}>
        ...
        <div className="confirm-overlay guide-tutorial-overlay" role="dialog" aria-modal="true" aria-label="操作ガイド">
          <div className="confirm-modal guide-tutorial-modal">
            ...
            <div className="tutorial-step-list">
              <article><span>1</span><strong>案件メールを貼ります</strong></article>
              <article><span>2</span><strong>AIが内容を整理します</strong></article>
              <article><span>3</span><strong>提案書と見積をダウンロードできます</strong></article>
            </div>
