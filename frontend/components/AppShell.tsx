...
import { CreationHistoryPanel } from "@/components/CreationHistoryPanel";
...
export default function Home() {
  ...
  useEffect(() => {
    ...
    return () => {
      isAppShellMountedRef.current = false;
      beautifulAiVerificationRequestRef.current += 1;
  ...
  async function refreshBeautifulAiVerification(hasUser = Boolean(getStoredUser())) {
    const requestId = ++beautifulAiVerificationRequestRef.current;
    logger.debug("beautiful_ai_status_probe:start", { requestId, hasUser });
    ...
    const isStale = requestId !== beautifulAiVerificationRequestRef.current;
    logger.debug("beautiful_ai_status_probe:finish", {
      requestId,
      hasUser,
      stale: isStale,
      configured: statusProbe?.status?.configured ?? null,
      enabled: statusProbe?.status?.enabled ?? null,
      errorType: statusProbe?.errorType ?? null
    });
    if (!isAppShellMountedRef.current || isStale) return;
    setBeautifulAiHealthProbe(healthProbe);
    setBeautifulAiStatusProbe(statusProbe);
    setBeautifulAiStatus(statusProbe?.status ?? null);
  }

  function clearBeautifulAiVerificationState() {
    beautifulAiVerificationRequestRef.current += 1;
  ...
  async function refreshAccountData() {
    ...
    if (storedUser) {
      ...
      clearBeautifulAiVerificationState();
      setWorkspaceContext(null);
      setWorkspaceContextError("");
    }
  ...
  const currentBeautifulAiStatus = beautifulAiStatus ?? beautifulAiStatusProbe?.status ?? null;
  const isBeautifulAiReady = Boolean(currentBeautifulAiStatus?.enabled);
  const isBeautifulAiConfigured = Boolean(currentBeautifulAiStatus?.configured);
  const isBeautifulAiMock = Boolean(currentBeautifulAiStatus?.mock ?? beautifulAiHealthProbe?.beautifulAiMock);
  ...
  const isBeautifulAiStatusAuthError =
    beautifulAiStatusProbe?.errorType === "auth_required" || beautifulAiStatusProbe?.errorType === "forbidden";
  ...
  const beautifulAiUnavailableMessage =
    ...
    currentBeautifulAiStatus?.message ||
    "Beautiful.ai status APIを確認中です。";
  const beautifulAiConfigurationMessage =
    !isBeautifulAiStatusApiReachable || !isBeautifulAiRouteFound
      ? beautifulAiUnavailableMessage
      : !isBeautifulAiConfigured
        ? "Beautiful.aiの設定が完了していません"
        : !isBeautifulAiReady
          ? "Beautiful.aiが有効になっていません"
          : "Beautiful.aiの状態を確認してください";
  const beautifulAiDiagnosticRows = useMemo(
    () => [
      ...
      },
      {
        ...
        required: true,
        note: beautifulAiStatusProbe?.message || (beautifulAiStatusProbe?.httpStatus ? `HTTP ${beautifulAiStatusProbe.httpStatus}` : "未確認")
      },
      {
        label: "Route Found",
      ...
      },
      {
        ...
        required: true,
        note: currentBeautifulAiStatus?.message || beautifulAiUnavailableMessage
      },
      {
        label: "Configured",
        ...
        required: true,
        note: isBeautifulAiConfigured ? "Backend側の設定あり" : beautifulAiConfigurationMessage
      },
      {
        label: "Mock",
      ...
      {
        label: "API Mode",
        value: currentBeautifulAiStatus?.api_mode || beautifulAiHealthProbe?.beautifulAiApiMode || "unknown",
        pass: true,
        required: false,
        note: (currentBeautifulAiStatus?.api_mode || beautifulAiHealthProbe?.beautifulAiApiMode) === "structured" ? "Structured APIを使用します。" : "Prompt APIを使用します。"
      },
      {
        label: "Maintenance",
  ...
  const proposalReviewItems = useMemo(() => {
    ...
    const isWebLike = /(Webサイト|サイトリニューアル|コーポレートサイト|ホームページ|CMS|SEO|WordPress|問い合わせフォーム)/i.test(sourceText);
    const projectFallback = isWebLike ? "Webサイト制作・改善" : "業務改善・導入提案";
    const purposeFallback = isWebLike ? "問い合わせ改善・信頼感向上" : "課題解決・業務効率化";
    const purposes = extractPurposeList(sourceText);
    const inferredValues = ["未定", "要確認", "競合未確認", "提案先企業", projectFallback, purposeFallback];
    ...
    return [
      ...
      { label: "案件内容", ...formatValue(extractedInfo?.projectContent || form.project_brief.slice(0, 80), projectFallback) },
      { label: "目的", ...formatValue(purposes.join("、"), purposeFallback) },
      { label: "予算", ...formatValue(form.budget_range, "未定") },
      { label: "納期", ...formatValue(form.desired_launch_timing, "要確認") },
      { label: isWebLike ? "CMS" : "要件", ...formatValue(isWebLike ? form.cms_required : form.special_function_required, "要確認") },
  ...
  const preGenerateCards = useMemo(() => {
    ...
    const isWebLike = /(Webサイト|サイトリニューアル|コーポレートサイト|ホームページ|CMS|SEO|WordPress|問い合わせフォーム)/i.test(allInputText(form));
    return [
      { label: "会社名", value: extractClientName(form) },
      { label: "案件内容", value: extractedInfo?.projectContent || (brief ? `${brief.slice(0, 110)}${brief.length > 110 ? "..." : ""}` : isWebLike ? "Webサイト制作・改善提案" : "業務改善・導入提案") },
      { label: "目的", value: purposes || (isWebLike ? "問い合わせ獲得・信頼感向上を仮説として整理" : "課題解決・業務効率化を仮説として整理") },
  ...
  async function handleCreateUser(payload: { email: string; password: string; role: CreatableUserRole; display_name?: string }) {
    await createUser(payload);
    const users = await listUsers();
    setManagedUsers(users.users);
  }

  async function handleUpdateUser(userId: number, payload: Parameters<typeof updateUser>[1]) {
...
競合：${nextExtracted.competitor || "未抽出"}`
  ...
  async function createBeautifulAiCurrent() {
    ...
    const pendingBeautifulAiWindow = openPendingBeautifulAiWindow();
    ...
    try {
      ...
      const beautifulAiUrl = getBeautifulAiOpenUrl(response);
      if (beautifulAiUrl) {
        if (!response.editor_url && response.player_url) {
          setBeautifulAiNotice(getUserFriendlyErrorMessage("BEAUTIFUL_AI_VIEW_ONLY_URL"));
        }
        const opened = openBeautifulAiUrl(beautifulAiUrl, response, pendingBeautifulAiWindow);
        if (!opened) {
          setBeautifulAiManualUrl(beautifulAiUrl);
          setBeautifulAiError(getUserFriendlyErrorMessage("POPUP_BLOCKED"));
        }
      } else {
        closePendingBeautifulAiWindow(pendingBeautifulAiWindow);
        setBeautifulAiError(getUserFriendlyErrorMessage("BEAUTIFUL_AI_URL_MISSING"));
      }
      trackEvent({
      ...
      closePendingBeautifulAiWindow(pendingBeautifulAiWindow);
      const friendly = toFriendlyError(caught);
      const message = `${getUserFriendlyErrorMessage(caught)} 既存PPTXも利用できます。`;
      setBeautifulAiError(message);
      setError(message);
  ...
  function openPendingBeautifulAiWindow() {
    if (typeof window === "undefined") return null;
    const popup = window.open("about:blank", "_blank");
    try {
      if (popup) {
        popup.opener = null;
        popup.document.title = "Beautiful.ai";
        popup.document.body.textContent = "Beautiful.aiを開いています...";
      }
    } catch {
      // The popup may be inaccessible depending on browser security settings.
    }
    return popup;
  }

  function closePendingBeautifulAiWindow(popup: Window | null) {
    try {
      if (popup && !popup.closed) popup.close();
    } catch {
      // Ignore browser popup lifecycle errors.
    }
  }

  function openBeautifulAiUrl(url: string, presentation: BeautifulAiPresentation | null = beautifulAiResult, popup: Window | null = null) {
    if (!url) {
      setBeautifulAiError(getUserFriendlyErrorMessage("BEAUTIFUL_AI_URL_MISSING"));
      return false;
    }
    if (presentation?.presentation_id) {
      void recordBeautifulAiEditorOpened(presentation.presentation_id).catch(() => undefined);
    }
    try {
  ...
  function fillSample(kind: SampleKind = "renewal") {
    ...
    const sampleClientInfo =
      kind === "renewal"
        ? "株式会社サンプル経理\n請求書処理と会計入力を月次で行う管理部門\n決裁者：管理部長、窓口：経理責任者"
        : kind === "recruit"
          ? "株式会社サンプル物流\n受注入力と出荷指示を日次で処理する業務部門\n決裁者：業務部長、窓口：情報システム部"
          : kind === "lp"
            ? "株式会社サンプル営業\n法人営業の案件管理と活動履歴を標準化したい営業部門\n決裁者：営業責任者、窓口：営業企画"
            : "株式会社サンプルWeb\nBtoBサービスの情報発信と問い合わせ導線を改善したい企業\n決裁者：営業責任者、窓口：マーケティング責任者";
    setEasyInput(sample);
    setForm((current) => ({
...
Web改善の重点：サービス内容、問い合わせ導線、更新体制を見直したい。
初回提案では、課題、構成、優先順位、KPI、概算費用を知りたい。`
            : buildProjectBriefFromEasyInput(sample),
        ...
        kind === "renewal"
          ...
          : kind === "recruit"
            ...
            : kind === "lp"
              ? "既存CRM/SFA"
              : "同業Webサービス",
      estimated_page_count: kind === "seo" ? "12ページ" : "",
      special_function_required: kind === "renewal" ? "AI-OCR、API/CSV連携、例外確認フロー" : kind === "recruit" ? "RPA、例外処理、監視" : kind === "lp" ? "CRM/SFA、項目設計、レポート" : "",
      content_creation_required: kind === "seo" ? "原稿作成一部あり" : "",
      hearing_result:
        kind === "renewal"
          ? "請求書の読取対象は毎月約1,000件。会社名、日付、金額、請求書番号を抽出し、CSVまたは会計システムへ連携したい。精度目標と例外確認フローは次回確認。"
          : kind === "recruit"
            ? "毎日2時間程度の転記作業があり、担当者によって処理手順が異なる。例外時の人手確認と監視方法も提案に含めたい。"
            : kind === "lp"
              ? "営業担当ごとに案件ステータスや次アクションの記録方法が異なる。初期設定、データ移行、定着支援まで相談したい。"
              : "問い合わせ導線と更新体制を改善したい。予算は100〜300万円。構成、コンテンツ優先順位、KPI、概算費用を知りたい。",
      own_service_info: "業務整理、導入設計、システム連携、運用定着支援、改善レポートを支援",
      past_proposal_template: "表紙、提案サマリー、現状理解、競合比較、ターゲット分析、導入戦略、導入構成、KPI、実行方針、スケジュール、体制、費用概算、今後の進め方",
      case_studies:
        kind === "recruit"
          ? "物流会社A：転記作業時間60%削減\n製造業B：確認漏れ30%削減"
          : kind === "lp"
  ...
  const guidedSummaryItems: GuidedSummaryItem[] = useMemo(
    () => {
      ...
      return [
        ...
        { label: "AI推測項目", value: [!form.budget_range ? "予算" : "", !form.desired_launch_timing ? "納期" : "", isWebLike && !form.cms_required ? "CMS" : "", !isWebLike && !form.special_function_required ? "導入要件" : ""].filter(Boolean).join("、") || "大きな推測項目はありません", inferred: true }
      ];
    },
    [extractedInfo?.budget, extractedInfo?.deadline, extractedInfo?.projectContent, extractedInfo?.trouble, form, rawSourceText, result?.analysis?.proposal_policy, result?.analysis?.quality_check?.human_review_notes]
  );
  const beautifulAiSimpleRequirements: BeautifulAiSimpleRequirement[] = useMemo(
    () => [
      ...
      { label: "提出前チェックを完了する", passed: isBeautifulAiQualityGateComplete, reason: "提出前チェックが完了していません" },
      {
        ...
        reason: beautifulAiConfigurationMessage
      },
    ...
    [beautifulAiConfigurationMessage, isBeautifulAiConfigured, isBeautifulAiOutputBusy, isBeautifulAiQualityGateComplete, isBeautifulAiReady, isBeautifulAiRoleAllowed, isBeautifulAiRouteFound, isBeautifulAiStatusApiReachable, isMaintenanceMode, result]
  );
  const beautifulAiStatusFailureReason =
    beautifulAiStatusProbe && !beautifulAiStatusProbe.status && !beautifulAiStatusProbe.apiReachable
      ? beautifulAiUnavailableMessage
      : "";
  const beautifulAiSimpleDisabledReason =
    beautifulAiStatusFailureReason ||
    beautifulAiSimpleRequirements.find((item) => !item.passed)?.reason ||
    "Beautiful.aiでプレゼンを作成できます";
    ...
    <main className={`app-shell ${isDarkMode ? "dark-mode" : ""} ${isGuidedDetailMode ? "guided-detail-mode" : "guided-normal-mode"}`}>
      ...
      {currentUser && (
        <GuidedFlow
          ...
          beautifulAiManualUrl={beautifulAiManualUrl}
          beautifulAiNotice={beautifulAiNotice}
        ...
        <article className="wizard-current-card wizard-chat-workspace">
          ...
          {result && (
            <div className="wizard-action-panel">
              ...
              {beautifulAiNotice && <p className="beautiful-ai-result" role="status">{beautifulAiNotice}</p>}
              {beautifulAiManualUrl && (
                <p className="beautiful-ai-result" role="status">
                  <a href={beautifulAiManualUrl} target="_blank" rel="noreferrer">Beautiful.aiを手動で開く</a>
                </p>
              )}
        ...
        <details className="wizard-effect-foldout">
          ...
          <div className="wizard-effect-grid">
            ...
            <article>
              ...
              <p>予算、納期、導入要件、競合などの不足情報を次回確認事項として整理します。</p>
            </article>
            <article>
        ...
        <div className="extractor-grid">
          <label className="field extractor-main-input">
            ...
            <textarea
              ...
              placeholder="例：
Ready Crew案件情報
株式会社サンプル経理様。AI-OCR導入希望。
請求書PDFから会社名、日付、金額、請求書番号を抽出したい。
予算は300万〜500万円。PoCは2〜3か月希望。
CSVまたは会計システムAPIへ連携したい。比較対象：既存OCRサービス"
              rows={9}
            />
          </label>
            ...
            {extractedInfo && (
              <article className="extractor-result-card">
                ...
                <dl>
                  ...
                  <div><dt>導入要件</dt><dd>{extractedInfo.cms || extractedInfo.inquiryDetails || "未抽出"}</dd></div>
                  <div><dt>ターゲット</dt><dd>{extractedInfo.target || "未抽出"}</dd></div>
                  <div><dt>効果測定</dt><dd>{extractedInfo.seoIssue || "未抽出"}</dd></div>
          ...
          <div className="business-mode-grid">
            <label className="field business-input-card">
              ...
              <textarea
                ...
                placeholder="例：本日の商談では、請求書AI-OCR、CSV連携、例外確認フローが主題。予算は次回確認。担当は営業が資料を作成し、開発側が概算見積を確認。"
                rows={12}
              />
            </label>
      ...
      <section className="workspace-grid">
        <form className="input-panel chat-input-panel" onSubmit={handleSubmit}>
          ...
          <section className="minimal-input-panel" aria-label="最小入力モード">
            ...
            <p className="minimal-input-note">未入力項目は「予算：未定」「納期：要確認」「導入要件：要確認」「競合：未確認」「決裁者：要確認」「ターゲット：要確認」で仮補完します。</p>
          </section>
          ...
          <section className="estimate-input-panel" aria-label="見積条件入力">
            <div className="check-panel-header">
              ...
              <span className="decision-pill rank-b">{estimateSummary.scopeLabel}</span>
            </div>
            <div className="estimate-field-grid">
              <label className="field">
                ...
                <input
                  ...
                  placeholder="例：対象画像数、認識カテゴリ数、12ページなど"
                />
              </label>
              <label className="field">
                <span>導入要件</span>
                <input
                  ...
                  placeholder="AI-OCR、RPA、CRM/SFA、Web/CMSなど"
                />
              </label>
              <label className="field">
                <span>確認画面・フォーム有無</span>
                <input
          ...
          <label className="field">
            ...
            <textarea
              ...
              placeholder="導入設計、システム連携、運用支援、AI-OCR、RPA、CRM/SFAなど"
              rows={4}
            />
          </label>
          ...
          <ProposalResultSection
            ...
            isBeautifulAiStatusAuthError={isBeautifulAiStatusAuthError}
