...
export default function Home() {
  ...
  const isBeautifulAiAdmin = currentUser?.role === "admin";
  const isBeautifulAiRoleAllowed = currentUser?.role === "admin" || currentUser?.role === "member";
  ...
  const beautifulAiDiagnosticRows = useMemo(
    () => [
      ...
      {
        label: "Quality Gate",
        value: qualityGateUnlocked ? "true" : "false",
        pass: qualityGateUnlocked,
        required: true,
        note: qualityGateUnlocked ? "提出前確認済み" : "提出前確認ゲートを完了してください。"
      },
      {
        label: "Admin",
        ...
        required: false,
        note: "adminは作成可能です。memberも作成できます。"
      },
      {
        label: "Role Allowed",
        ...
        required: true,
        note: currentUser?.role ? `現在の権限: ${currentUser.role}` : "ログイン状態を確認してください。"
      },
      {
        label: "Status API",
...
初回提案では、SEO課題、サイト構造、コンテンツ優先順位、KPI、概算費用を知りたい。`
  ...
  const renderBeautifulAiDiagnosticsPanel = () => (
    <section className="beautiful-ai-diagnostics" aria-label="Beautiful.aiボタンの利用条件" data-testid="beautiful-ai-disabled-reasons">
