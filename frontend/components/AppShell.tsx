...
初回提案では、SEO課題、サイト構造、コンテンツ優先順位、KPI、概算費用を知りたい。`
  ...
  return (
    ...
    <main className={`app-shell ${isDarkMode ? "dark-mode" : ""}`}>
      ...
      <section className="compact-intro-panel" aria-label="利用案内">
        <div>
          <p className="eyebrow">Start Here</p>
          <h2>案件メールを貼るだけで、提案書・見積・商談準備までAIが整理します。</h2>
        </div>
        <div className="quick-action-row">
          <button className="primary-button" type="button" onClick={autoPrepareProposal}>
            <Sparkles size={18} aria-hidden="true" />
            AIに全部おまかせ
          </button>
          <button className="secondary-button" type="button" onClick={() => document.getElementById("case-input-panel")?.scrollIntoView({ behavior: "smooth" })}>
            案件メールを貼る
          </button>
          <button className="secondary-button" type="button" onClick={() => setInputMode("easy")}>
            最小入力
          </button>
          <button className="secondary-button" type="button" onClick={() => setIsConfirmOpen(true)} disabled={!canSubmit || !canGenerate}>
            提案書を生成
          </button>
        </div>
      </section>
...
CMSはWordPress希望。競合：https://example.co.jp"
      ...
      <details className="future-integration-panel advanced-foldout" aria-label="今後の拡張予定">
