...
def get_quality_gate(db: Connection, project_id: str) -> dict[str, Any] | None:
...
def save_quality_gate(db: Connection, project_id: str, user_id: int, checklist_items: list[str]) -> dict[str, Any]:
    ...
    existing = get_quality_gate(db, project_id)
    if existing:
        db.execute(
            """
            ...
            WHERE project_id = ?
            """,
            (user_id, encoded_items, project_id),
        )
    else:
        db.execute(
            """
            INSERT INTO quality_gates (project_id, user_id, checklist_items)
            VALUES (?, ?, ?)
            """,
            (project_id, user_id, encoded_items),
        )
        create_audit_log(db, user_id, "quality_gate_start", "quality_gate", project_id, "success", "started")
    return get_quality_gate(db, project_id) or {}
...
def complete_quality_gate(db: Connection, project_id: str, user_id: int, checklist_items: list[str]) -> dict[str, Any]:
    ...
    encoded_items = json.dumps(items, ensure_ascii=False)
    if get_quality_gate(db, project_id):
        db.execute(
            """
            UPDATE quality_gates
            SET user_id = ?,
        ...
        db.execute(
            """
            INSERT INTO quality_gates (project_id, user_id, checklist_items, completed, completed_at)
...
def bypass_quality_gate(db: Connection, project_id: str, user_id: int, bypass_reason: str) -> dict[str, Any]:
    ...
    if get_quality_gate(db, project_id):
        db.execute(
            ...
            WHERE project_id = ?
            """,
            (user_id, reason, project_id),
        )
    else:
        db.execute(
            """
            INSERT INTO quality_gates (project_id, user_id, bypassed, completed_at, bypass_reason)
            VALUES (?, ?, 1, CURRENT_TIMESTAMP, ?)
            """,
