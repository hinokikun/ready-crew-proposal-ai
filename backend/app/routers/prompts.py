...
async def get_dashboard(_: dict = Depends(require_roles("admin", "manager"))) -> dict:
    with get_db() as db:
        return {"dashboard": get_prompt_studio_dashboard(db)}
...
async def post_prompt_version(payload: PromptVersionCreateRequest, user: dict = Depends(require_roles("admin", "manager"))) -> dict:
    with get_db() as db:
        ...
        return {"prompt_version": prompt_version, "dashboard": get_prompt_studio_dashboard(db)}
...
async def post_experiment(
    ...
    with get_db() as db:
        ...
        return {"experiment": experiment, "dashboard": get_prompt_studio_dashboard(db)}
...
async def post_experiment_judgement(
    ...
    with get_db() as db:
        ...
        return {"recommendation": recommendation, "dashboard": get_prompt_studio_dashboard(db)}
...
async def post_from_learning(
    ...
    with get_db() as db:
        ...
        return {"result": result, "dashboard": get_prompt_studio_dashboard(db)}
...
async def post_prompt_metric(
    ...
    _: dict = Depends(require_roles("admin", "manager", "member")),
    __: None = Depends(rate_limit_dependency("generation")),
) -> dict:
    with get_db() as db:
        metric = record_prompt_metric_from_payload(db, payload=payload)
        return {"metric": metric, "analytics": get_prompt_studio_dashboard(db)["analytics"]}
