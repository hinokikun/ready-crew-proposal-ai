...
def get_review_by_project(db: Connection, project_id: str) -> dict[str, Any] | None:
...
def list_reviews(db: Connection, limit: int = 100) -> list[dict[str, Any]]:
    rows = db.execute(
        """
        SELECT r.*, creator.email AS creator_email, reviewer.email AS reviewer_email
        FROM proposal_reviews r
...
def request_review(db: Connection, project_id: str, project_name: str, creator_user_id: int) -> dict[str, Any]:
    ...
    existing = db.execute("SELECT id FROM proposal_reviews WHERE project_id = ? ORDER BY updated_at DESC, id DESC LIMIT 1", (safe_project_id,)).fetchone()
    if existing:
        db.execute(
            """
        ...
        cursor = db.execute(
            ...
            (project_id, project_name, creator_user_id, status, review_requested_at)
            VALUES (?, ?, ?, 'review_requested', CURRENT_TIMESTAMP)
            """,
            (safe_project_id, safe_project_name, creator_user_id),
        )
    ...
    return dict(db.execute("SELECT * FROM proposal_reviews WHERE id = ?", (review_id,)).fetchone())
...
def update_review_status(db: Connection, review_id: int, reviewer_user_id: int, status: str, review_comment: str) -> dict[str, Any] | None:
    ...
    db.execute(
        ...
        WHERE id = ?
        """,
        (status, safe_comment, reviewer_user_id, review_id),
    )
    review = _row_to_dict(db.execute("SELECT * FROM proposal_reviews WHERE id = ?", (review_id,)).fetchone())
    if review:
...
def apply_review_feedback(db: Connection, review_id: int, user_id: int, current_summary: str = "") -> dict[str, Any] | None:
    ...
    db.execute(
        ...
        (review_id, project_id, previous_status, next_status, review_comment, ai_improvement_policy, diff_summary, executed_by_user_id)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?)
        """,
        (
            review_id,
    ...
    updated = _row_to_dict(db.execute("SELECT * FROM proposal_reviews WHERE id = ?", (review_id,)).fetchone())
    return {
        "review": updated,
...
def request_review_again(db: Connection, review_id: int, user_id: int) -> dict[str, Any] | None:
    ...
    return _row_to_dict(db.execute("SELECT * FROM proposal_reviews WHERE id = ?", (review_id,)).fetchone())


def list_review_revisions(db: Connection, review_id: int) -> list[dict[str, Any]]:
    rows = db.execute(
        """
